...
### 1. 抓取评论信息
页面上的评论呈列表分布，每一项为一个独立的评论区块（Comment Block）。
*   **定位评论块**：使用通用的列表项选择器（建议按具有 `comment-item` 属性或特定层级的 `div` 进行遍历）。
*   **提取用户名**：用户名通常紧邻用户头像。
    *   **XPath 建议**：`//span[contains(@class, 'avatar')]/following-sibling::text()[1]` 或根据具体的 CSS 类获取。
*   **提取评论文本**：获取评论内容所在的 `div` 的内文本，或者获取图片表情包的 `img` 属性。
